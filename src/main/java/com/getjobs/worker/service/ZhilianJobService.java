...
public class ZhilianJobService implements JobPlatformService {
    ...
    public void executeDelivery(Consumer<JobProgressMessage> progressCallback) {
        ...
        try {
            ...
            // 投递中心的 platform_configs 覆盖（如有）
            java.util.Map<String, Object> overrides = com.getjobs.delivery.service.DeliveryConfigOverrideHolder.get();
            if (overrides != null && !overrides.isEmpty()) {
                com.getjobs.delivery.service.ConfigOverrideApplier.applyZhilian(config, overrides);
                log.info("Zhilian 配置已按投递中心 platform_configs 覆盖：keywords={} city={} salary={}", config.getKeywords(), config.getCityCode(), config.getSalary());
            }
