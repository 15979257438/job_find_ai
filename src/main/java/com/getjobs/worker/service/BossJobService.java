...
public class BossJobService implements JobPlatformService {
    ...
    public void executeDelivery(Consumer<JobProgressMessage> progressCallback) {
        ...
        try {
            // 加载配置 + 应用投递中心 overrides（即使没登录也要应用，便于审计）
            BossConfig config = configService.getBossConfig();
            java.util.Map<String, Object> overrides = com.getjobs.delivery.service.DeliveryConfigOverrideHolder.get();
            if (overrides != null && !overrides.isEmpty()) {
                com.getjobs.delivery.service.ConfigOverrideApplier.applyBoss(config, overrides);
                log.info("Boss 配置已按投递中心 platform_configs 覆盖：keywords={} city={} waitTime={}", config.getKeywords(), config.getCityCode(), config.getWaitTime());
            }
